...
# def getMatches(conn, wemail): #buggy for two sided
...
#     return curs.fetchall()
def getMatches(conn, userEmail): #works perfect now
    ...
    twoSidedMatching = []
    for person in people: #maybe there's a better way for two sided
...
def getOneSidedMatches(conn, userEmail): 
    ...
    if len(twoSidedMatches) > 0: 
        for oneSDMatch in possibleOneSided: #efficiency issue
